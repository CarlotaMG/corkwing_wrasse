...
abundance_matrix <- read.table("data/DE/abundance_matrix.txt", header = TRUE, row.names = 1)
...
for (combo in pc_combos) {
  ...
  p <- ggplot(pcaData, aes_string(
    ...
    ggtitle(paste0("PCA Colored Temperature (PC", x_pc, " vs PC", y_pc, ")")) +
    theme_minimal() +
  ...
  plots[[paste0("PC", x_pc, "_vs_PC", y_pc)]] <- p
}
...
for (combo in pc_combos) {
  ...
  p <- ggplot(pcaData, aes_string(
    x = paste0("PC", x_pc),
    y = paste0("PC", y_pc),
    color = "origin",       # <-- origin as color
    shape = "temperature"   # <-- temperature as shape
  )) +
    geom_point(size = 3) +
    xlab(paste0("PC", x_pc, ": ", percentVar[x_pc], "% variance")) +
    ylab(paste0("PC", y_pc, ": ", percentVar[y_pc], "% variance")) +
    ggtitle(paste0("PCA Colored Origin (PC", x_pc, " vs PC", y_pc, ")")) +
    theme_minimal() +
