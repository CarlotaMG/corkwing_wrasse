...
summarize_DE <- function(files_dir) {
  files <- list.files(files_dir, pattern = "_DE_results.csv$", full.names = TRUE)
  summary <- data.frame()
  
  for (f in files) {
    df <- read_csv(f)
    comp <- gsub("_DE_results.csv", "", basename(f))
...
# Summaries for temperature and origin contrasts
temp_summary <- summarize_DE("results/DE/temperature_contrast_condition_model")
origin_summary <- summarize_DE("results/DE/origin_contrast_condition_model")
...
write_csv(temp_summary, "results/DE/temperature_contrast_condition_model/summary_table.csv")
write_csv(origin_summary, "results/DE/origin_contrast_condition_model/summary_table.csv")
```
